...
class Camera
{
  Camera()
  {
    position      = new PVector( 0, 0, 600 ); // initial position
  ...
  void HandleKeyPressed()
  {
    ...
    if ( key == 'r' || key == 'R' ){
      // resets camera so its not contunint to spin
  ...
  void HandleKeyReleased()
  {
    ...
    if ( keyCode == SHIFT ){
      shiftPressed = false;
      positiveMovement.mult(0);
      negativeMovement.mult(0);
    }
  }
